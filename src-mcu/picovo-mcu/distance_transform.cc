...
static uint32_t lut_bitmap[16] __attribute__((section(".itcm_data")));

// LUT for dt stage 1
static uint32_t lut_dt_s1[256] __attribute__((section(".itcm_data")));
...
void distance_transform(uint8_t *input, uint8_t *output, int width, int height)
{
  ...
  for (int x = 0; x < width; x += 8) {
    ...
    for (int y = 0; y < height; y++, pin += (width>>3), pout += width) {
