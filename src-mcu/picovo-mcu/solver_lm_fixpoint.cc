...
static int32_t lut_residual[256] __attribute__((section(".itcm_data")));
// LUT for 1/(huber*sqrt(uint8_t))
static int32_t lut_invres[256] __attribute__((section(".itcm_data")));
// LUT for gradients: 0.5*f*huber*sqrt(uint8_t)
static int16_t lut_iu[256] __attribute__((section(".itcm_data")));
static int16_t lut_iv[256] __attribute__((section(".itcm_data")));
// the LM lamda
static float lamda;
...
)
{
  ...
  const int16_t ri00 = R(0,0) * ((1L << 15) - 1);
  const int16_t ri01 = R(0,1) * ((1L << 15) - 1);
  const int16_t ri02 = R(0,2) * ((1L << 15) - 1);
  const int16_t ri10 = R(1,0) * ((1L << 15) - 1);
  const int16_t ri11 = R(1,1) * ((1L << 15) - 1);
  const int16_t ri12 = R(1,2) * ((1L << 15) - 1);
  const int16_t ri20 = R(2,0) * ((1L << 15) - 1);
  const int16_t ri21 = R(2,1) * ((1L << 15) - 1);
  const int16_t ri22 = R(2,2) * ((1L << 15) - 1);
  const int16_t ti0 = T(0) * (1L << 15);
  const int16_t ti1 = T(1) * (1L << 15);
  const int16_t ti2 = T(2) * (1L << 15);

  // Fixed-point convertion for camera intrinsics
  const int32_t fx_i = CAM_FX * (1L << 6);  // Q10.6
  const int32_t fy_i = CAM_FY * (1L << 6);  // Q10.6
  const int32_t cx_i = (CAM_CX + 0.5) * (1L << 18); // Q9.18
  const int32_t cy_i = (CAM_CY + 0.5) * (1L << 18); // Q9.18
