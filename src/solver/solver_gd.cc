...
)
{
  ...
  if (!is_pre_calc_kf) {
    // use normal estimator
    last_min_resdl = _calc_update_gd(ref_dt, cur_pcl, cur_frame->nr_tracked, 
      huber_weight * _outlier_threshold, R, T, jacobian);
  } else {
    // use pre-calculated gradient
    last_min_resdl = _calc_update_gd_uv(ref_dt2, ref_dt_indexes, 
      cur_pcl, cur_frame->nr_tracked, 
      huber_weight * _outlier_threshold, R, T, jacobian);
  }
  ...
  for (; its < 100; its++) {
    Vector6 inc = jacobian / (1+lamda) / 2500.0;
    ...
    if (!is_pre_calc_kf) {
      // use normal estimator
      cur_resdl = _calc_update_gd(ref_dt, cur_pcl, cur_frame->nr_tracked,
        huber_weight * _outlier_threshold,
        new_ref2cur.rotationMatrix(), new_ref2cur.translation(),
        jacobian);
    } else {
      // use pre-calculated gradient
      cur_resdl = _calc_update_gd_uv(ref_dt2, ref_dt_indexes, 
        cur_pcl, cur_frame->nr_tracked,
        huber_weight * _outlier_threshold,
        new_ref2cur.rotationMatrix(), new_ref2cur.translation(),
        jacobian);
...
float solver_gd::_calc_update_gd(
...
# endif
    ...
    jacobian[0] += Iu * z_inv;
    jacobian[1] += Iv * z_inv;
    jacobian[2] +=  -Iu_x_z_Iv_y_z * z_inv;
    jacobian[3] += -(Iu_x_z_Iv_y_z * y_z + Iv);
    jacobian[4] +=  (Iu_x_z_Iv_y_z * x_z + Iu);
    jacobian[5] += -Iu * y_z + Iv * x_z;

    total_res += residual;
    nr_valid_proj++;
  }

  jacobian /= nr_valid_proj;

  // return number of valid projections
  nr_proj = nr_valid_proj;

# ifdef DEBUG_OPTIMIZATION
  std::cout << "  J= " << jacobian.transpose() << ", " 
...
float solver_gd::_calc_update_gd_uv(
