...
float solver_lm_nn::track_frame(
...
)
{
  ...
  uint8_t *ref_dt_sign = ref_frame->dt_to_grad_sign;
  ...
  last_min_resdl = _calc_update_lm(ref_dt_lut, ref_dt_sign, cur_pcl, 
  ...
  for (; its < 100; its++) {
    ...
    cur_resdl = _calc_update_lm(ref_dt_lut, ref_dt_sign, cur_pcl + featoffset, 
    ...
    } else if ((res_inc_history & 0xf) == 0xf) {
#ifdef DEBUG_OPTIMIZATION
...
float solver_lm_nn::_calc_update_lm(
...
# ifdef DEBUG_OPTIMIZATION
    cv::circle(proj_ref, cv::Point(u, v), 0, cv::Scalar(0,255,0));
# endif
    ...
    uint8_t sign = *(ref_dt_sign + (pixindex >> 2)) >> ((pixindex & 0x3) << 1);
    const struct dt_grad_f &proj_dt = _grad_lut[lutindex];
    float residual = proj_dt.residual;
    // if (residual > 5) {
    //   continue;
    // }

    float Iu = sign & 0x2 ? -proj_dt.Iu : proj_dt.Iu;
    float Iv = sign & 0x1 ? -proj_dt.Iv : proj_dt.Iv;
    ...
    Eigen::Matrix<float, 6, 2> J62;
    J62.setZero();
    J62(0,0) = fx * z_inv;
    J62(2,0) = -fx * x_z * z_inv;
    J62(3,0) = -fx * x_z * y_z;
    J62(4,0) = fx + fx * x_z * x_z;
    J62(5,0) = -fx * y_z;
    J62(1,1) = fy * z_inv;
    J62(2,1) = -fy * y_z * z_inv;
    J62(3,1) = -fy - fy * y_z * y_z;
    J62(4,1) = fy * x_z * y_z;
    J62(5,1) = fy * x_z;

    // H += J*J.transpose() / residual;
    H += J62*J62.transpose();// / residual * huber_weight;
    b += J;
...
# ifdef DEBUG_OPTIMIZATION
  std::cout << "  H= " << H.row(0) << " " << H.row(1)  << " " << H.row(2)
    ...
    " Hp= " << Hp.row(0) << " " << Hp.row(1)  << " " << Hp.row(2)
    << " " << Hp.row(3) << " " << Hp.row(4)  << " " << Hp.row(5)
    << "\n b= " << b.transpose() << "\n " 
    << nr_proj << " projections, residual " << total_res / nr_valid_proj << std::endl;
